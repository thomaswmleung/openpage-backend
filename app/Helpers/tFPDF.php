...
use App\Helpers\TTFontFile;

define('tFPDF_VERSION', '1.24');
define('FPDF_FONTPATH', realpath("font"));
...
//    $this->fontpath = FPDF_FONTPATH.DIRECTORY_SEPARATOR;
//}
    ...
    function __construct() {
        ...
        if (defined('FPDF_FONTPATH')) {
            ...
            if (substr($this->fontpath, -1) != DIRECTORY_SEPARATOR)
                $this->fontpath .= DIRECTORY_SEPARATOR;
    ...
    function AddFont($family, $style = '', $file = '', $uni = false) {
        ...
        if ($uni) {
            if (defined("_SYSTEM_TTFONTS") && file_exists(_SYSTEM_TTFONTS . $file)) {
                ...
                $ttffilename = $this->_getfontpath() . 'unifont' . DIRECTORY_SEPARATOR . $file;
            }
            $unifilename = $this->_getfontpath() . 'unifont' . DIRECTORY_SEPARATOR . strtolower(substr($file, 0, (strpos($file, '.'))));
            $name = '';
            $originalsize = 0;
            ...
            if (!isset($type) || !isset($name) || $originalsize != $ttfstat['size']) {
                $ttffile = $ttffilename;
//			require_once($this->_getfontpath().'unifont'.DIRECTORY_SEPARATOR.'ttfonts.php');
                $ttf = new TTFontFile();
    ...
    function Write($h, $txt, $link = '',$fillcolor = FALSE) {
        ...
        while ($i < $nb) {
            ...
            if ($c == "\n") {
                // Explicit line break
                if ($this->unifontSubset) {
                    $this->Cell($w, $h, mb_substr($s, $j, $i - $j, 'UTF-8'), 0, 2, '', $fillcolor, $link);
                } else {
                    $this->Cell($w, $h, substr($s, $j, $i - $j), 0, 2, '', $fillcolor, $link);
                }
                $i++;
                $sep = -1;
                $j = $i;
            ...
            if ($l > $wmax) {
                ...
                if ($sep == -1) {
                    ...
                    if ($this->unifontSubset) {
                        $this->Cell($w, $h, mb_substr($s, $j, $sep - $j, 'UTF-8'), 0, 2, '', $fillcolor, $link);
                    } else {
                        $this->Cell($w, $h, substr($s, $j, $sep - $j), 0, 2, '', $fillcolor, $link);
                    }
                    $i = $sep + 1;
                }
                $sep = -1;
        ...
        if ($i != $j) {
            if ($this->unifontSubset) {
                $this->Cell($l, $h, mb_substr($s, $j, $i - $j, 'UTF-8'), 0, 0, '', $fillcolor, $link);
            } else {
                $this->Cell($l, $h, substr($s, $j), 0, 0, '', $fillcolor, $link);
            }
        }
    }
...
//            $cw = $this->CurrentFont[ 'cw' ][ $c ] * ( $this->FontSizePt / 1000 );
    ...
    function WriteInFlowMode($width, $height, $content) {

        $tempW = $this->w;
        $tempLMargin = $this->lMargin;
        $tempRMargin = $this->RMargin;


        $this->w = $width;
        $this->lMargin = $this->x;
        $this->rMargin = $this->x + $width;

        $this->Write($height, $content);

        $this->w = $tempW;
        $this->lMargin = $this->lMargin;
        $this->rMargin = $this->RMargin;
    }
